...
; Village Map Exporter
        ; if you wish to save your map out in a different style than the default, you can easily select one of the 6 presets
        ; that are available on the Villgae Generator page and then return to this dialog and click 'Process URLs'.
Gui, Font, s12
Gui, Add, Text, w400 x30, You can set up the Village Generator's style settings before you process the URLs from your FMG map.`n`nTo do this, just open the Village Generator webpage by clicking the upper right button, choose your style settings, and then return to this dialog and click 'Process URLs.`n`nYou can also view the keyboard shortcuts for the Village Generator by clicking the button in the upper left below.`n`nYou will see a dialog message displayed after the first URL is processed, asking you if everything is working as expected.`n`nThe message will timeout after 8 seconds and process the next URL.`n`nYou will have options on that message to cancel or hide the message and continue processing the rest of the URLs.

Gui, Add, Button, x80 y+20 w150 h40 gOpenKSURL, Village Generator`nKeyboard Shortcuts
Gui, Add, Button, x+20 w150 h40 gOpenURL, Open Village Generator page
Gui, Add, Button, x80 y+10 w150 h40 gCancel, Cancel
Gui, Add, Button, x+20 w150 h40 gOK, Process URLs

Gui, Show, w460 h580, Villages Maps Downloader
...
OpenKSURL:
    Run, chrome.exe --profile-directory="Default" --new-window https://watabou.itch.io/village-generator/devlog/426543/keyboard-shortcuts
return

OpenURL:
    Run, chrome.exe --profile-directory="Default" --new-window https://watabou.github.io/village-generator
return
...
OK:
    Gui, Destroy
            ...
            Sleep, sleepShort
            ; OPTION TO CONFIGURE - CHROME WINDOW SIZE
            ...
            Loop, Read, %FilePath% ; process URLS from FilePath
                {
                    ...
                    url := A_LoopReadLine ; Get the current URL from the file
                    ...
                    RemoveToolTip(10000) ; Remove tooltip after 4 seconds
                    ...
                    Sleep sleepDeep
                    ; now we will save the village map as a PNG file
                    ; postion cursor
                    MouseMove, 250, 700
                    ; send Right Click
                    Click, right
            ...
            BlockInput, Off
            MSGBOX,, Villages Script Finished, All the FMG Burgs-Villages URLs have been processed successfully.`n`nThe files have downloaded to your default download location.`n`nMove them to your campaign's asset folder in your Obsidian vault BEFORE you run the other AHK script.
return
