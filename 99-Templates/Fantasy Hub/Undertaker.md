<%*
// Fantasy Hub — Undertaker (standalone template — no Town Forge needed).
...
const name = (await api.rollUnscoped("UndertakerName")).result;
...
tR += `---\ntype: undertaker\nsize: "${size}"\ntown: "${town}"\n---\n`;
tR += `# ${name}\n\n> [!info] undertaker in ${town}\n\nAn undertaker on the streets of ${town}.\n\n`;
...
const result = await api.rollUnscoped("TF-Undertaker", { promptValues: {
  town, shopType: "undertaker", shopName: name, size,
