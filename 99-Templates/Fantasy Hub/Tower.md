<%*
// Fantasy Hub — Mage Tower (standalone template — no Town Forge needed).
...
const name = (await api.rollUnscoped("TowerName")).result;
...
tR += `---\ntype: tower\nsize: "${size}"\ntown: "${town}"\n---\n`;
tR += `# ${name}\n\n> [!info] tower in ${town}\n\nA tower on the streets of ${town}.\n\n`;
...
const result = await api.rollUnscoped("TF-Tower", { promptValues: {
  town, shopType: "tower", shopName: name, size,
