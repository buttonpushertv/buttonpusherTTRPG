<%*
// Fantasy Hub — Inn (standalone template — no Town Forge needed).
...
const name = (await api.rollUnscoped("InnName")).result;
...
tR += `---\ntype: inn\nsize: "${size}"\ntown: "${town}"\n---\n`;
tR += `# ${name}\n\n> [!info] inn in ${town}\n\nAn inn on the streets of ${town}.\n\n`;

// The innkeeper — same person in the infobox and the rolled text.
let main = null;
if (has) { main = await P.roll(); tR += infobox(main, "Innkeeper", "Keeps"); }

const result = await api.rollUnscoped("TF-Inn", { promptValues: {
  town, shopType: "inn", shopName: name, size,
