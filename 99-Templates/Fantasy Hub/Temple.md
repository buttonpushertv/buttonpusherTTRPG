<%*
// Fantasy Hub — Temple (standalone template — no Town Forge needed).
...
const name = (await api.rollUnscoped("TempleName")).result;
...
tR += `---\ntype: temple\nsize: "${size}"\ntown: "${town}"\n---\n`;
tR += `# ${name}\n\n> [!info] temple in ${town}\n\nA temple on the streets of ${town}.\n\n`;
...
const result = await api.rollUnscoped("TF-Temple", { promptValues: {
  town, shopType: "temple", shopName: name, size,
