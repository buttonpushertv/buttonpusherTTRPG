<%*
// Fantasy Hub — Barracks (standalone template — no Town Forge needed).
...
const name = (await api.rollUnscoped("BarracksName")).result;
...
tR += `---\ntype: barracks\nsize: "${size}"\ntown: "${town}"\n---\n`;
tR += `# ${name}\n\n> [!info] barracks in ${town}\n\nA barracks guarding ${town}.\n\n`;
...
const result = await api.rollUnscoped("TF-Barracks", { promptValues: {
  town, shopType: "barracks", shopName: name, size,
