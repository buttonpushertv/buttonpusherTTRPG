<%*
// Fantasy Hub — Dock (standalone template — no Town Forge needed).
...
const name = (await api.rollUnscoped("DockName")).result;
...
const result = await api.rollUnscoped("TF-Dock", { promptValues: {
  town, shopType: "dock", shopName: name, size,
