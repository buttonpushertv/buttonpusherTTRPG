<%*
// Fantasy Hub — Market (standalone template — no Town Forge needed).
...
const name = (await api.rollUnscoped("MarketName")).result;
...
tR += `---\ntype: market\nsize: "${size}"\ntown: "${town}"\n---\n`;
tR += `# ${name}\n\n> [!info] market in ${town}\n\nA market in the heart of ${town}.\n\n`;

// The market has no single proprietor — the faces below are the crowd.
const result = await api.rollUnscoped("TF-Market", { promptValues: {
  town, shopType: "market", shopName: name, size
}});
tR += result.result;
