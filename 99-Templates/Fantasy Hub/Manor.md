<%*
// Fantasy Hub — Manor (standalone template — no Town Forge needed).
...
const name = (await api.rollUnscoped("ManorName")).result;
...
tR += `---\ntype: manor\nsize: "${size}"\ntown: "${town}"\n---\n`;
tR += `# ${name}\n\n> [!info] manor in ${town}\n\nA manor on the streets of ${town}.\n\n`;
...
const result = await api.rollUnscoped("TF-Manor", { promptValues: {
  town, shopType: "manor", shopName: name, size,
